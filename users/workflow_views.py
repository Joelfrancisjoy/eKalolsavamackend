...
def student_group_profile(request, group_entry_id):
    ...
    if group_entry is None:
        return Response({'error': 'Group entry not found or not authorized'}, status=status.HTTP_404_NOT_FOUND)
    ...
    try:
        with transaction.atomic():
            _apply_group_entry_profile_update(group_entry, validated_data)
...
def school_update_group_participant(request, group_entry_id):
    if getattr(request.user, 'role', None) != 'school':
        return Response({'error': 'Only school users can access this resource'}, status=status.HTTP_403_FORBIDDEN)
    ...
    if group_entry.status == 'approved':
        return Response(
            {'error': 'Approved group entries cannot be edited from the school dashboard'},
...
def school_individual_events(request):
    ...
    if has_level_filter != has_gender_filter:
        return Response(
            {'error': 'student_class or level_code and gender_category must be provided together'},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def school_group_events(request):
    ...
    if bool(group_class) != bool(gender_category):
        return Response(
            {'error': 'group_class and gender_category must be provided together'},
            status=status.HTTP_400_BAD_REQUEST,
        )
    if group_class not in GROUP_CLASS_CHOICES:
...
def admin_approve_school_group_participant(request, group_entry_id):
    ...
    leader_member = group_entry.members.filter(is_leader=True).order_by('member_order').first()
    if leader_member is None:
        return Response({'error': 'Leader details are missing for this group entry'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    try:
        ...
        return Response({
            'error': 'Failed to create group leader account: username or other unique field already exists.',
            'details': str(exc),
        }, status=status.HTTP_409_CONFLICT)
    except Exception as exc:
        return Response({'error': f'Failed to create group leader account: {str(exc)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
